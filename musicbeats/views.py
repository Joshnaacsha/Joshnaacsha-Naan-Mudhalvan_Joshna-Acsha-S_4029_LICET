...
def upload(request):
    if request.method == "POST":
        ...
        song_model = Song(name=name, singer=singer, tags=tag,
                          image=image, movie=movie, credit=credit, song=song1)
        song_model.save()
...
def search(request):
    if 'q' in request.GET:
        ...
        if query:
            ...
            results = Song.objects.filter(name__icontains=query)
    ...
    # Return an empty array if no results found or if query is empty
